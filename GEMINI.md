...
- **Concise Plain-Text Commit Messages vs Rich Detailed PRs**:
  - **Git Commit Messages**: Keep commit messages short, concise, and focused (singkat dan padat). Follow Conventional Commits (`<type>(<scope>): <subject>`) with an optional brief 1-3 line plain-text explanation of WHY. Never bloat commit messages with exhaustive component listings or step-by-step issue walk-throughs. Do NOT use Markdown formatting syntax (no `#`, `##`, `**`, or code fences) because terminal logs (`git log`) and GitHub `<pre>` monospace blocks do not render Markdown.
  - **GitHub Pull Request Descriptions**: Place the comprehensive, detailed breakdown of changes, architectural rationale, component updates, and test verification matrices inside the Pull Request description, where rich GitHub Flavored Markdown (GFM) (headings, tables, links, code blocks) is fully supported. Exclude local developer tooling status (such as `codegraph sync`) from PR descriptions and commit messages, as local indexing tools are meant strictly for individual developer workstations and are not project deliverables.
- **Language Policy**:
