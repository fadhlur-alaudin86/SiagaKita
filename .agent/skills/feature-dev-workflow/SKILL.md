...
| 7 | **Review Gate & Retrospective** — Run automated quality gatekeeper (`python3 scripts/verify_pipeline.py`) across all 7 gates + conduct self-review audits (Security, Database, Silent-Failure, Dead-Code). Execute Step 7.5 Workflow Retrospective (apply minor skill updates or draft major proposals). Ensure CI passes. Sync issue checklist. Open PR targeting `dev`. | Quality scorecard + audit report + workflow updates + Pull Request |
| 8 | **Close Log & Evolution** — Update feature log (all steps completed). Run `python3 scripts/sync_backlog_status.py --fix` for backlog & plan parity + run `python3 scripts/reconcile_issue_status.py` for post-merge issue & parent tracker reconciliation. Record applied workflow evolutions. Link PR. Move issue to Done. | Updated feature log + reconciled trackers |
...
14. **Conventional Commits & Concise Plain-Text Formatting MANDATORY** — Format: `<type>(<scope>): <description>`. Keep commit messages short and focused (singkat dan padat) with an optional brief 1-3 line plain-text explanation of WHY. Never include exhaustive issue task lists, component inventories, Markdown headers (`##`), bold (`**`), or code fences in commit messages. Full detailed breakdowns belong in the Pull Request description (excluding local developer tooling status such as `codegraph sync`).
...
34. **Step 7 → Pre-PR Verification Gate & Retrospective (MANDATORY)**:
    - **Unified Gatekeeper Mandate (MANDATORY)**: Never commit or open a PR based on piecemeal manual checks alone. The agent MUST execute `python3 scripts/verify_pipeline.py` (or `--fast` during intermediate iterations) across all 7 pre-flight quality gates: (1) OpenAPI contracts in `docs/api/`, (2) database migration sequence and `docs/DATABASE_SCHEMA.md` synchronization, (3) localization dictionary hygiene (`scripts/check_localization_orphans.py --all`), (4) code formatting (`gofmt` & `dart format`, auto-formatted with `--format`), (5) static analysis, linters & security (`golangci-lint`, `go vet`, `govulncheck`, and `flutter analyze`), (6) regression unit test suite (`go test ./... -race` and `flutter test`), and (7) backlog & planning documentation parity (`scripts/sync_backlog_status.py --check`). All 7 gates must pass cleanly before opening a PR.
    - **Self-Review Checklist**: Review all changed code against [review-standards.md](review-standards.md) (Security Review, Database & Migration Review, Silent Failure Audit, Clean Code & Dead Code Elimination, and Karpathy Behavioral Invariants).
...
34a. **Non-Interactive CLI Standard**: When executing `gh` CLI commands (especially `gh run view`), always pass explicit target IDs (`gh run view <run_id> --log-failed`) or non-interactive flags. Never invoke commands that trigger interactive terminal selection menus (`? Select a workflow run`), which freeze non-TTY agent background tasks.
34b. **Git Pre-Commit Hook Standard**: All developer and agent environments must run `scripts/install_git_hooks.sh` once upon setup so `.git/hooks/pre-commit` automatically formats staged Go (`gofmt -w`) and Dart (`dart format`) files on commit, guaranteeing clean formatting before reaching remote CI.
35. **Step 8 → Done & Closed**:
    - **Automated Issue & Parent Tracker Reconciliation (`reconcile_issue_status.py`)**: Merging into `dev` triggers automated reconciliation via CI (`ci-dev.yml` / `issue-status-labeler.yml`) and can be run locally via `python3 scripts/reconcile_issue_status.py`. It updates merged sub-issues to `status: done`, removes obsolete status labels, autocompletes remaining markdown checkboxes (`- [x]`), closes child issues, and evaluates parent tracking issues to mark subtasks `- [x]` and update their status to `status: ready` once all child issues are resolved.
    - **Living Documentation Sync (`sync_backlog_status.py --fix`)**: Run `python3 scripts/sync_backlog_status.py --fix` to verify and align living documentation parity across `.planning/` architectural plans, `docs/backlog/features/`, and GitHub issues.
    - **Parent Tracker Sync**: If working under a parent issue, ensure the corresponding subtask checkbox (`- [ ]` to `- [x]`) in the parent issue body is checked.
