...
9. **Parent Tracker Automation** — When all child sub-tasks under a parent tracking issue are completed and merged, automation marks all parent checklist boxes to `[x]` and transitions the parent issue label to `status: ready`. Final issue closure (`gh issue close`) is reserved for tech lead / human sign-off after full verification.
